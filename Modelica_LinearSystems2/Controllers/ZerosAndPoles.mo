...
block ZerosAndPoles
  ...
  import Modelica_LinearSystems2.Controllers.Internal;
...
initial equation
  if continuous then
    if init ==Modelica_LinearSystems2.Controllers.Types.Init.InitialState then
      x = x_start;
    elseif init ==Modelica_LinearSystems2.Controllers.Types.Init.SteadyState then
      der(x) = zeros(nx);
    elseif init ==Modelica_LinearSystems2.Controllers.Types.Init.InitialOutput and nx>0 then
      y = y_start;
...
<pre>      <strong>der</strong>(y) = c*<strong>der</strong>(x) + d*<strong>der</strong>(u)
...
<pre>   <strong>der</strong>(x) = -b*x + u
        y = x
</pre>
<p>
Differentiating y symbolically leads to:
</p>
<pre>     <strong>der</strong>(y) = <strong>der</strong>(x)
