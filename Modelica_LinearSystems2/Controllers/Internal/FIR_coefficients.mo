...
function FIR_coefficients "Calculates the FIR-filter coefficient vector"
  ...
  input FIRspec specType=Modelica_LinearSystems2.Controllers.Types.FIRspec.MeanValue
  ...
  input Types.Window window=Modelica_LinearSystems2.Controllers.Types.Window.Rectangle
    "Type of window" annotation(Dialog(enable=specType==FIRspec.Window));
  input Real beta=2.12 "Beta-Parameter for Kaiser-window"
    annotation(Dialog(enable=specType==FIRspec.Window and window==Modelica_LinearSystems2.Controllers.Types.Window.Kaiser));
...
algorithm
  assert(f_cut<=1/(2*Ts),
    "The cut-off frequency f_cut may not be greater than half the sample frequency (Nyquist frequency), i.e. f_cut <= " + String(1/(2*Ts)) + " but is "+String(f_cut));
  ...
  elseif specType == FIRspec.Window then
    ...
    for i in 1:order2 + 1 loop
      ...
      if i - 1 == order2/2 then
        ...
        a[i] := if filterType == FilterType.LowPass then sin(k*Wc)*w[i]/(k*pi) else
                w[i]*(sin(k*pi) - sin(k*Wc))/(k*pi);
      end if;
    end for;
  else
    a := a_desired;
