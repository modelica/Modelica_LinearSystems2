...
<p>Block <b>SampleClock</b> defines options for all components of the Controllers library that are on the same or on a lower level as the sampleClock component. In every block, the default defined in SampleClock can be changed. The following options are available:</p>
