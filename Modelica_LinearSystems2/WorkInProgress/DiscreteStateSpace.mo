...
record DiscreteStateSpace
  ...
  function fromDiscreteZerosAndPoles =
     ...
     */
        (LU,pivots) := MatricesMSL.LU(identity(nx) -
          Ts*sc.A);
        sd.B2 := MatricesMSL.LU_solve2(
              LU,
              pivots,
              Ts*sc.B);
        sd.A := MatricesMSL.LU_solve2(
              LU,
              pivots,
              identity(nx));
        sd.B := sd.A*sd.B2;
     ...
     */
      ...
      elseif method == Method.StepExact then
       ...
       y = C*x + D*u
    */
        (sd.A,sd.B) := MatricesMSL.integralExp(
              sc.A,
              sc.B,
              Ts);
        sd.C := sc.C;
  ...
  z = x - gamma1/Ts*u
    ...
    */
        (sd.A,sd.B,sd.B2) := MatricesMSL.integralExpT(
              sc.A,
              sc.B,
              Ts);
        sd.B2 := sd.B2/Ts;
...
is defined via equation &quot; = <strong>pre</strong>(new_x)&quot;.
</p>
<p>
