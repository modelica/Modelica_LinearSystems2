...
model SRKF_inner "Discrete State Space block"
  ...
  final parameter Real Cq[size(dss.B, 2),size(dss.B, 2)]=MatricesMSL.cholesky(Q,false)
    "Input or process noise covariance matrix of the previous instant";
  ...
  final parameter Real Cr[size(dss.C, 1),size(dss.C, 1)]=MatricesMSL.cholesky(R,false)
    "Output or measurement noise covariance matrix of the previous instant";
...
public
  Real S[size(dss.A, 1),size(dss.A, 1)](start=MatricesMSL.cholesky(P0,false))
...
//  Real K1[size(dss.A, 1),size(dss.C, 1)] "Kalman filter gain matrix";
  ...
  Real M[size(dss.C, 1)+size(dss.A, 1), 2*size(dss.C, 1)+size(dss.A, 1)];
...
initial equation
  S=MatricesMSL.cholesky(P0,false);
  x_est=x_init;

equation
 when sample(0, sampleTime) then
    z = dss.A*pre(x_est) + dss.B*pre(u);
    (K,S, Cr_out,M) = Modelica_LinearSystems2.WorkInProgress.DiscreteStateSpace.Design.sr_kfStepMatrices(
                                                                  dss.A, dss.C, pre(S), wB, Cq, Cr);
