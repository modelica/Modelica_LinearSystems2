...
partial function timeResponseMask
  ...
  input Integer toWindow[:,:]=fill(-1, size(sc.C, 1), size(sc.B, 2))
    "-1/0/>0 plot in new window/last window/window ID";
  /* the Dialog annotation seems not to work properly. why? */
  input Boolean clearWindow[:,:]=fill(false, size(sc.C, 1), size(sc.B, 2))
    "True, if previous window content is removed" annotation(Dialog(enable=max(toWindow)>=0));
  input String heading[:,:]=fill("Time response", size(sc.C, 1), size(sc.B, 2))
