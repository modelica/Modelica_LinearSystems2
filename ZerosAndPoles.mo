...
// ssD = [0],
    ...
    algorithm
     ...
     //first elements of A, B, C and D
       ...
       // Construct state space systems of order 1
          a := -den[1, 1];
          b := if abs(den[1, 1])>Modelica.Constants.small then den[1,1] else num[1,1];

          if 1 <= n_num1 then
                // State space system in form (4)
            c := if abs(den[1, 1])>Modelica.Constants.small then k[1]*(num[1, 1] - den[1, 1])/den[1, 1] else k[1];
            d := k[1];
            dZero := false;
          else
                // State space system in form (5)
