...
//  zp = 1/( (p + 1)*(p + 2) )
  ...
  function toStateSpace
      "Transform a DiscreteZerosAndPoles object into a StateSpace object"
    import Modelica;
    import Modelica_LinearSystems2;
    import Modelica_LinearSystems2.WorkInProgress.DiscreteZerosAndPoles;
    ...
    output DiscreteStateSpace dss(
      ...
      redeclare Real B2[DiscreteZerosAndPoles.Analysis.denominatorDegree(dzp),1])
        "Transfer function in StateSpace SISO form";
    ...
    Integer nx=DiscreteZerosAndPoles.Analysis.denominatorDegree(dzp);
    ...
    Integer i;
    Integer ili;
    Real num[nx,2]=[dzp.n2; [dzp.n1,zeros(n_num1)]; zeros(nx - n_num2 - n_num1, 2)]
        "Numerator matrix, in order that indices are defined in all situations in all if clauses";
    Real den[nx,2]=[dzp.d2; [dzp.d1,zeros(n_den1)]; zeros(nx - n_den2 - n_den1, 2)]
  ...
  algorithm
   ...
   //  for i in max(2,i_d):n_den1 loop
      ...
      dss := Modelica_LinearSystems2.WorkInProgress.DiscreteStateSpace(r=dzp.k);
    end if;
...
<h4><font color=\"#008000\">Syntax</font></h4>
<table>
<tr> <td align=right>  dss </td><td align=center> =  </td>  <td> DiscreteZerosAndPoles.Conversion.toStateSpace<b>toStateSpace</b>(tf)  </td> </tr>
</table>
<h4><font color=\"#008000\">Description</font></h4>
<p>
...
Based on this representation, evrey block with transfer function G(q) could be transformed into
<blockquote><pre>
  G(q) = k * F(q)

</pre>
with F(q) has unit gain. This leads to representations of the forms
</pre></blockquote>
<p>
<blockquote><pre>

           a2 + a1*q + q^2       a2      b2 + a1*b2/a2*q + b2/a2*q^2
  G(q) = -------------------- = ---- * ------------------------------ = k * F(q),  k = a2/b2  (1)

           b2 + b1*q + q^2       b2           b2 + b1*q + q^2
&nbsp;
...
<blockquote><pre>
(1)
          a2 + a1*q + q^2           der(x1) = x2
    y = ---------------------  -->  der(x2) = -b2*x1 - b1*x2 + b2*u
          b2 + b1*q + q^2                 y = c*((a2-b2)*x1 + (a1-b1)*x2 + u),  c = b2/a2
&nbsp;
(2)
             q + a                 der(x1) = x2
    y = ---------------- * u  -->  der(x2) = -b2*x1 - b1*x2 + b2*u
        b2 + b1*q + q^2                  y = k*(a1/b2*x1 +x2/b2),  c = b2/a
&nbsp;
(3)
               1                  der(x1) = x2
    y = --------------- *u   -->  der(x2) = -b2*x1 - b1*x2 + b2*u
        b2 + b1*q + q^2                 y = c*x1/b2,  c = b2
&nbsp;
(4)
       a + q                       der(x) = -b*x + b*u
...
<b>algorithm</b>
  dss := Modelica_LinearSystems2.DiscreteZerosAndPoles.Conversion.toStateSpace(dzp);
// dss.A = [0, 1; -1, -1],
// dss.B = [0; 1],
// dss.C = [1, 1],
// dss.D = [0],
